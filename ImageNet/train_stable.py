...
def train_cuda(model, device, train_loader, optimizer, criterion, batch_size, args, y_log, target_log):
    ...
    args.num_of_batches_for_mean = 1
    args.num_of_layers_with_nodeStableLoss = 1
    for batch_idx, (data, target) in enumerate(train_loader):
        target = torch.squeeze(target)
        data, target = data.to(device), target.to(device)
        model = model.to(device)
        optimizer.zero_grad()
        prediction, intermedia_y = model(data)
        ...
        if (len(y_log[0])<args.num_of_batches_for_mean):
            ...
            for layer_idx in layer_list_for_backward:
                y_log[layer_idx].append(intermedia_y[layer_idx].clone().detach())
        ...
        for layer_idx in layer_list_for_backward:
            ...
            if layer_idx == args.num_of_layers_with_nodeStableLoss - 1:
                ...
                square_difference_the_class = (intermedia_y[layer_idx] - y_log_tensor_mean_scattered.to(device)) **2
