...
def main():
    ...
    validset, trainset = torch.utils.data.random_split(trainset, [args.num_valid, 50000 - args.num_valid])
    ...
    for epoch in range(training_epochs):
        ...
        print("{}\t{:.4f}\t{:.4f}".format(epoch, valid_loss, valid_accuracy))
        ...
        if is_test:
            torch.save(model, "./result/" + args.model_name + f"_{args.lambda_balance}_{args.random_seed}.pt")
            test_loss, test_correct, test_accuracy = test(model, device, test_loader, criterion)
